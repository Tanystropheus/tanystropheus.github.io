...
function addActionToLibrary(a) {
	...
	newAction.find('.del-col').click(function(){
		var response=confirm('Remove this action from the collection');
		if(!response)
			return;
		var cid=this.parentNode.parentNode.id;
		var i=indexOfCollection(cid);
		this.parentNode.parentNode.removeChild(this.parentNode);
		myvoice.collection[i].actions.splice(indexOfAction(myvoice.collection[i].actions,a),1);
		
		var newAction=$('.'+a.id); // todo: replace by $(this)?
		newAction.find('.add-col').attr('disabled',false);
	});
	newAction.find('.del-lib').click(function(){
		var response=confirm("Remove this action from the library");
		if(!response)
			return;
		$(".action."+a.id).remove();
		myvoice.library.splice(indexOfAction(myvoice.library,a),1);
		for(var i in myvoice.collection) {
			var j=indexOfAction(myvoice.collection[i].actions,a);
			if(j!=undefined)
				myvoice.collection[i].actions.splice(i,1);
		}
	});
	newAction.find('.edit-lib').click(function(){
		alert("[This should allow to edit the action, its icon for example]")
	});
	newAction.find('.add-col').click(function() {
		// add action to collection
		var selcid=myvoice.state.selectedCollection;
		var i=indexOfCollection(selcid);
		if(indexOfAction(myvoice.collection[i].actions,a)>=0) {
			alert("This action exists already in collection");
			return;
		}
		addActionToCollection(a,selcid);
	});
}
...
function drop(ev) {
	...
	var action=myvoice.library[indexOfAction(myvoice.library,{id:$("#"+srcid).data().id})];
	var destName;
